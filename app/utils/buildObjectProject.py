...
def build_object_project(email_content: str) -> tuple[Project | None, bool]:
    ...
    if project_url is None:
        logger.info("Cannot extract project URL from email content")
        return None, False
    crawler = CodeurProjectCrawler(project_url)
    if not crawler.check_project_availability():
        logger.info("Project %s is not available", project_url)
        return None, False
    ...
    logger.info("Build object project %s successfully", project_url)
    return project, True